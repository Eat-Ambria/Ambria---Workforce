...
// ─── VIDEO MODAL ──────────────────────────────────────────────────────────────
function VideoModal({video,isDone,onClose,onCompleted,lang}){
  const H=lang==="hi";
    ...
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}}
      ...
      <div style={{background:C.white,borderRadius:14,width:"100%",maxWidth:700,overflow:"hidden",position:"relative"}}>
        ...
        <div style={{background:`linear-gradient(135deg,${C.maroon},${C.maroonLight})`,padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
          ...
          <button onClick={onClose} style={{width:32,height:32,borderRadius:"50%",border:"1px solid rgba(255,255,255,0.4)",background:"rgba(255,255,255,0.15)",color:"#fff",cursor:"pointer",fontSize:17,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>✕</button>
        </div>
        ...
        {/* Completion toast */}
        {toast&&<div style={{position:"absolute",bottom:18,left:"50%",transform:"translateX(-50%)",background:C.green,color:"#fff",borderRadius:10,padding:"9px 22px",fontFamily:F.b,fontSize:13,fontWeight:700,whiteSpace:"nowrap",boxShadow:"0 4px 14px rgba(0,0,0,0.3)",zIndex:10}}>
          ✅ {H?"प्रशिक्षण पूर्ण!":"Training completed!"}
        </div>}
      </div>
    </div>
  );
                ...
                <div style={{padding:"7px 9px 8px",background:watched?"#F0FFF4":C.white}}>
                  ...
                  {isAdmin&&<div style={{display:"flex",gap:4}} onClick={e=>e.stopPropagation()}>
                    <button onClick={()=>{setEV(v);setSF(true);window.scrollTo({top:0,behavior:"smooth"});}}
                      style={{flex:1,padding:"4px 0",borderRadius:5,border:`1px solid ${C.border}`,background:C.white,cursor:"pointer",fontSize:9,fontWeight:600,color:C.tl}}>✏️ Edit</button>
                    <button onClick={()=>deleteVideo(v.id)}
                      style={{flex:1,padding:"4px 0",borderRadius:5,border:"none",background:C.rBg,cursor:"pointer",fontSize:9,fontWeight:600,color:C.red}}>🗑️</button>
                  </div>}
                </div>
              </div>
            );
