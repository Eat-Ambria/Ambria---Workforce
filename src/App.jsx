...
// ═══ UI ═══
function Bdg({children,color=C.tl,bg=C.border+"88"}){return <span style={{display:"inline-flex",alignItems:"center",gap:3,padding:"3px 8px",borderRadius:6,fontSize:11,fontWeight:600,background:bg,color,whiteSpace:"nowrap",fontFamily:F.b}}>{children}</span>;}
...
function SearchSelect({value,onChange,options,style:cs,placeholder}){
  ...
  return(<div ref={ref} style={{position:"relative",...cs}}>
    <button onClick={()=>{setOpen(p=>!p);setQ("");}} style={{width:"100%",minWidth:80,padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border}`,background:C.white,fontFamily:F.b,fontSize:13,color:C.text,cursor:"pointer",outline:"none",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center",gap:4}}>
...
function LoginScreen({onLogin,lang,setLang}){
  ...
  return(<div style={{minHeight:"100vh",background:`linear-gradient(135deg,${C.maroon},${C.maroonLight},#2D1520)`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:F.b,padding:20}}>
    <div style={{width:"100%",maxWidth:380,background:C.white,borderRadius:20,padding:36,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}><button onClick={()=>setLang(lang==="en"?"hi":"en")} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${C.border}`,background:C.bg,fontFamily:F.b,fontSize:12,cursor:"pointer",fontWeight:600,color:C.maroon}}>{lang==="en"?"हिंदी":"English"}</button></div>
      <div style={{textAlign:"center",marginBottom:28}}><div style={{width:60,height:60,borderRadius:"50%",background:`linear-gradient(135deg,${C.maroon},${C.maroonLight})`,display:"inline-flex",alignItems:"center",justifyContent:"center",fontFamily:F.d,color:C.white,fontSize:30,fontWeight:700,marginBottom:10}}>A</div><h1 style={{fontFamily:F.d,fontSize:21,fontWeight:700,color:C.maroon,margin:"0 0 4px",lineHeight:1.2}}>Ambria Work Force</h1></div>
      <div style={{marginBottom:14}}><label style={{fontSize:13,fontWeight:600,marginBottom:5,display:"block"}}>{L.username}</label><input type="text" value={u} onChange={e=>sU(e.target.value)} placeholder={L.enterUser} onKeyDown={e=>{if(e.key==="Enter")go();}} style={{width:"100%",padding:"12px 16px",borderRadius:10,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:15,outline:"none",boxSizing:"border-box",background:C.bg}}/></div>
      <div style={{marginBottom:12}}><label style={{fontSize:13,fontWeight:600,marginBottom:5,display:"block"}}>{L.password}</label><div style={{position:"relative"}}><input type={sh?"text":"password"} value={p} onChange={e=>sP(e.target.value)} placeholder={L.enterPass} onKeyDown={e=>{if(e.key==="Enter")go();}} style={{width:"100%",padding:"12px 16px",paddingRight:44,borderRadius:10,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:15,outline:"none",boxSizing:"border-box",background:C.bg}}/><button onClick={()=>sSh(!sh)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",border:"none",background:"none",cursor:"pointer",fontSize:17}}>{sh?"🙈":"👁️"}</button></div></div>
      <label style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,cursor:"pointer",fontSize:13,color:C.tl}}><div onClick={()=>setRem(!rem)} style={{width:18,height:18,borderRadius:4,border:`2px solid ${rem?C.maroon:C.border}`,background:rem?C.maroon:C.white,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>{rem&&<span style={{color:C.white,fontSize:12,fontWeight:700}}>✓</span>}</div>{lang==="hi"?"पासवर्ड याद रखें":"Remember me"}</label>
      {err&&<div style={{background:C.rBg,color:C.red,padding:"10px",borderRadius:8,fontSize:13,marginBottom:14}}>{err}</div>}
      <button onClick={go} disabled={loading} style={{width:"100%",padding:"13px",borderRadius:10,border:"none",background:loading?"#9A2E42":C.maroon,color:C.white,fontFamily:F.b,fontSize:16,fontWeight:700,cursor:loading?"not-allowed":"pointer",opacity:loading?0.8:1}}>{loading?"...":L.login}</button>
    </div></div>);
}
...
function PhotoUp({photos,onUp,onRetake,disabled,L}){
  ...
  return(<div>
    {!has?(<div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={()=>ref.current?.click()} disabled={disabled} style={{padding:"10px 16px",borderRadius:10,border:`2px dashed ${!disabled?C.accent:C.border}`,background:!disabled?"#FFF7ED":C.bg,cursor:disabled?"default":"pointer",fontFamily:F.b,fontSize:13,fontWeight:700,color:!disabled?C.accent:C.tl,opacity:disabled?0.5:1}}>📸 {L.uploadPhoto}</button>{!disabled&&<span style={{fontSize:11,color:C.red,fontWeight:600}}>⚠️ {L.photoNeeded}</span>}</div>)
    :(<div><div style={{display:"inline-flex",flexDirection:"column",borderRadius:12,overflow:"hidden",border:`2px solid ${C.green}`}}><img src={photos[0].data} alt="" style={{width:120,height:120,objectFit:"cover"}}/><div style={{background:"rgba(0,0,0,0.75)",color:C.white,padding:"4px 8px",display:"flex",justifyContent:"space-between"}}><span style={{fontSize:10}}>📸 {photos[0].time}</span><span style={{fontSize:8}}>{photos[0].date}</span></div></div>
      {!disabled&&<div style={{marginTop:8}}><button onClick={()=>{onRetake();setTimeout(()=>ref.current?.click(),100);}} style={{padding:"8px 14px",borderRadius:8,border:`1px solid ${C.yellow}`,background:C.yBg,cursor:"pointer",fontFamily:F.b,fontSize:12,fontWeight:700,color:C.yellow}}>🔄 {L.retakePhoto}</button></div>}</div>)}
...
function TC({task:t,uTask,delTask,depts,areas,user:u,allM,L,lang}){
  ...
  return(<div style={{background:dn?C.gBg:iss?C.rBg:C.white,borderRadius:12,border:`1px solid ${dn?"#b8dcc8":iss?"#f0c8c4":C.border}`,overflow:"hidden",borderLeft:`4px solid ${({daily:C.blue,weekly:C.green,monthly:C.maroon})[t.cat]||C.tl}`}}>
    <div style={{display:"flex",alignItems:"center"}}>
      {canA&&<button onClick={hDn} style={{width:46,minHeight:60,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",background:dn?C.green:"transparent",flexShrink:0,borderRight:`1px solid ${dn?"#5aad7a":C.border}`}}><div style={{width:22,height:22,borderRadius:6,border:dn?"none":`2.5px solid ${C.border}`,background:dn?"rgba(255,255,255,0.3)":C.white,display:"flex",alignItems:"center",justifyContent:"center"}}>{dn&&<span style={{color:C.white,fontSize:14,fontWeight:700}}>✓</span>}</div></button>}
      ...
      <div style={{flex:1,padding:"8px 10px",minWidth:0}}>
        <div style={{fontSize:14,fontWeight:600,color:dn?C.green:C.text,textDecoration:dn?"line-through":"none",marginBottom:3}}>{ti}</div>
        <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap",marginBottom:3}}><span style={{fontSize:11,color:dn?"#5a9a72":C.tl}}>{area?.i} {area?.n}</span><span style={{padding:"2px 7px",borderRadius:5,background:C.bBg,color:C.blue,fontSize:11,fontWeight:700}}>⏱{t.dur}</span></div>
        <div style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 8px",borderRadius:6,background:dn?"#b8dcc840":"#FFF7ED",border:`1px solid ${dn?"#b8dcc8":"#f0dcc8"}`}}>
          <span style={{fontSize:11}}>{t.isTeam?"👥":"🕐"}</span><span style={{fontSize:11,fontWeight:700,color:dn?C.green:C.accent}}>{t.timeBlock}</span>
          {t.isTeam&&<span style={{fontSize:10,fontWeight:600,color:C.blue,background:C.bBg,padding:"1px 5px",borderRadius:4}}>{L.teamTask}</span>}
        </div>
        {dn&&t.completedAt&&<div style={{fontSize:10,color:C.green,marginTop:2}}>✅ {new Date(t.completedAt).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})} - {t.completedBy}</div>}
        {t.photos?.length>0&&<div style={{fontSize:10,color:C.blue,marginTop:1}}>📸 photo</div>}
        {t.notes&&<div style={{fontSize:10,color:C.tl,marginTop:1,fontStyle:"italic",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:250}}>💬 {t.notes.split("\n").pop()}</div>}
      </div>
      <div style={{display:"flex",alignItems:"center",gap:3,padding:"6px",flexShrink:0}}>
        ...
        {canA&&!dn&&<button onClick={()=>uTask(t.id,{status:iss?"pending":"issue"})} style={{width:24,height:24,borderRadius:6,border:"none",cursor:"pointer",background:iss?C.red:C.bg,fontSize:11,display:"flex",alignItems:"center",justifyContent:"center"}}>⚠️</button>}
        <button onClick={()=>setSC(!sC)} style={{width:24,height:24,borderRadius:6,border:"none",cursor:"pointer",background:sC?C.maroonSoft:C.bg,fontSize:11,display:"flex",alignItems:"center",justifyContent:"center"}}>💬</button>
        <button onClick={()=>setSS(!sS)} style={{width:24,height:24,borderRadius:6,border:"none",cursor:"pointer",background:sS?C.maroonSoft:C.bg,fontSize:10,display:"flex",alignItems:"center",justifyContent:"center"}}>📖</button>
        {isA&&<button onClick={()=>setSE(!sE)} style={{width:24,height:24,borderRadius:6,border:"none",cursor:"pointer",background:sE?C.maroonSoft:C.bg,fontSize:10,display:"flex",alignItems:"center",justifyContent:"center"}}>✏️</button>}
      </div>
    </div>
    {sS&&<div style={{padding:"0 10px 10px 50px",borderTop:`1px solid ${dn?"#b8dcc8":C.border}`}}><div style={{margin:"8px 0",padding:10,background:C.bg,borderRadius:8,fontSize:13,lineHeight:1.7}}><div style={{fontWeight:700,fontSize:11,color:C.maroon,marginBottom:3}}>{L.sop}</div>{de}</div><PhotoUp photos={t.photos||[]} onUp={(p)=>uTask(t.id,{photos:[p]})} onRetake={()=>uTask(t.id,{photos:[]})} disabled={dn} L={L}/></div>}
    {sC&&<div style={{padding:"0 10px 10px 50px",borderTop:sS?"none":`1px solid ${dn?"#b8dcc8":C.border}`}}>
      {t.notes&&<div style={{margin:"6px 0",padding:8,background:C.bg,borderRadius:8,fontSize:11,lineHeight:1.8,whiteSpace:"pre-line",maxHeight:80,overflowY:"auto"}}>{t.notes}</div>}
      <div style={{display:"flex",gap:6,marginTop:6}}><input type="text" placeholder={L.comment} value={cT} onChange={e=>setCT(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")subC();}} style={{flex:1,padding:"7px 12px",borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:13,outline:"none"}}/><Btn2 primary small onClick={subC}>{L.send}</Btn2></div></div>}
    {sE&&isA&&<div style={{padding:"8px 10px 8px 50px",borderTop:`1px solid ${C.border}`,background:C.maroonSoft}}>
      <div style={{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}}>
        ...
        {delTask&&<button onClick={()=>delTask(t.id)} style={{padding:"5px 8px",borderRadius:8,border:"none",background:C.rBg,color:C.red,fontFamily:F.b,fontSize:11,fontWeight:600,cursor:"pointer"}}>🗑️</button>}
      </div></div>}
  </div>);
}
...
function AddTF({prop,onAdd,onClose,L}){
  ...
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      <input placeholder={L.taskTitle+" (EN)"} value={f.title} onChange={e=>sF({...f,title:e.target.value})} style={{gridColumn:"1/-1",padding:10,borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:14,outline:"none"}}/>
      <input placeholder={L.taskTitle+" (HI)"} value={f.titleHi} onChange={e=>sF({...f,titleHi:e.target.value})} style={{gridColumn:"1/-1",padding:10,borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:14,outline:"none"}}/>
      ...
      <input placeholder="Time" value={f.timeBlock} onChange={e=>sF({...f,timeBlock:e.target.value})} style={{padding:8,borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:13}}/>
      <textarea placeholder={L.desc+" (EN)"} value={f.desc} onChange={e=>sF({...f,desc:e.target.value})} style={{gridColumn:"1/-1",padding:8,borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:13,minHeight:40,resize:"vertical"}}/>
    </div>
...
function Sidebar({view,setView,user:u,effectiveUser,onLogout,lang,setLang,nC,setShowN,L,pm,setPM,pAs,setPAs,allDbUsers,dirs,aP}){
  ...
  return(<div style={{width:185,background:C.white,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",height:"100vh",position:"fixed",left:0,top:0,zIndex:50}}>
    <div style={{padding:"14px 12px",borderBottom:`1px solid ${C.border}`}}>
      <div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.maroon},${C.maroonLight})`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:F.d,color:C.white,fontSize:17,fontWeight:700}}>A</div><div style={{fontFamily:F.d,fontSize:14,fontWeight:700,color:C.maroon,lineHeight:1.2}}>Ambria Work Force</div></div>
      {isA&&<button onClick={()=>setShowN(true)} style={{marginTop:8,width:"100%",padding:"5px 8px",borderRadius:8,border:`1px solid ${nC>0?C.accent:C.border}`,background:nC>0?"#FFF7ED":C.bg,cursor:"pointer",fontFamily:F.b,fontSize:11,fontWeight:600,color:nC>0?C.accent:C.tl}}>🔔 {L.notif}{nC>0&&<span style={{background:C.red,color:C.white,borderRadius:10,padding:"0 5px",fontSize:10,marginLeft:4}}>{nC}</span>}</button>}
    </div>
    <div style={{flex:1,padding:"8px 6px",display:"flex",flexDirection:"column",gap:2}}>
      {nav.map(n=><button key={n.id} onClick={()=>setView(n.id)} style={{display:"flex",alignItems:"center",gap:7,padding:"8px 10px",borderRadius:8,border:"none",cursor:"pointer",fontFamily:F.b,fontSize:13,fontWeight:view===n.id?600:400,background:view===n.id?C.maroonSoft:"transparent",color:view===n.id?C.maroon:C.tl,textAlign:"left",position:"relative"}}>
        <span style={{fontSize:14}}>{n.i}</span><span style={{flex:1}}>{n.l}</span>
        {n.badge>0&&<span style={{background:C.red,color:C.white,borderRadius:10,padding:"1px 6px",fontSize:10,fontWeight:700,minWidth:16,textAlign:"center"}}>{n.badge}</span>}
      </button>)}
      {isSA&&<div style={{marginTop:6,padding:8,background:pm?C.bBg:C.bg,borderRadius:8,border:`1px solid ${pm?C.blue:C.border}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:pm?6:0}}>
          <span style={{fontSize:10,fontWeight:700,color:pm?C.blue:C.tl}}>👁️ {pm?L.previewOff:L.preview}</span>
    ...
    <div style={{padding:"8px 6px",borderTop:`1px solid ${C.border}`}}>
      <button onClick={()=>setLang(lang==="en"?"hi":"en")} style={{width:"100%",padding:"5px",borderRadius:8,border:`1px solid ${C.border}`,background:C.bg,fontFamily:F.b,fontSize:11,cursor:"pointer",fontWeight:600,color:C.maroon,marginBottom:5}}>🌐 {lang==="en"?L.hi:L.en}</button>
      <div style={{display:"flex",alignItems:"center",gap:5,padding:"5px 6px",background:C.bg,borderRadius:8,marginBottom:5}}><div style={{width:24,height:24,borderRadius:"50%",background:C.maroon,display:"flex",alignItems:"center",justifyContent:"center",color:C.white,fontWeight:700,fontSize:11}}>{u.name[0]}</div><div><div style={{fontSize:11,fontWeight:600}}>{u.name}</div><div style={{fontSize:8,color:C.tl}}>{rL[u.role]}</div></div></div>
      <button onClick={onLogout} style={{width:"100%",padding:"6px",borderRadius:8,border:`1px solid ${C.red}`,background:C.rBg,color:C.red,fontFamily:F.b,fontSize:11,fontWeight:600,cursor:"pointer"}}>🚪 {L.logout}</button>
    </div></div>);
}

function NPanel({ns,onClose,onClr,L,onClickNotif}){return(<div style={{position:"fixed",top:0,right:0,width:320,height:"100vh",background:C.white,boxShadow:"-4px 0 20px rgba(0,0,0,0.1)",zIndex:100,display:"flex",flexDirection:"column"}}><div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:F.d,fontSize:16,fontWeight:700,color:C.maroon}}>🔔 {L.notif}</span><div style={{display:"flex",gap:4}}><Btn2 small onClick={onClr}>{L.clearAll}</Btn2><button onClick={onClose} style={{border:"none",background:"none",cursor:"pointer",fontSize:15}}>✕</button></div></div>
  <div style={{flex:1,overflowY:"auto",padding:8}}>{ns.length===0?<div style={{textAlign:"center",padding:24,color:C.tl,fontSize:12}}>{L.noNotif}</div>:ns.map((n,i)=>{
    ...
    return(<div key={i} onClick={()=>{if(isApproval&&onClickNotif){onClickNotif(n);onClose();}}} style={{padding:10,borderRadius:8,background:bgC,marginBottom:5,cursor:isApproval?"pointer":"default",border:isApproval?`2px solid ${C.accent}`:"none"}}>
      <div style={{fontSize:11,fontWeight:600,color:txC}}>{isApproval?"🔔 Approval Request":n.type==="issue"?"⚠️":"✅"} {n.task}</div>
      <div style={{fontSize:10,marginTop:2}}>{n.by} · {n.prop} · {n.time}</div>
      {isApproval&&<div style={{fontSize:10,color:C.accent,marginTop:3,fontWeight:700}}>👆 Tap to review & approve</div>}
    </div>);})}</div></div>);}
...
function AttView({user:u,att,setAtt,prop,L}){
  ...
  return(<div><h1 style={{fontFamily:F.d,fontSize:21,fontWeight:700,color:C.maroon,margin:"0 0 12px"}}>🕐 {L.attendance} - {prop.sn}</h1>
    <input ref={attRef} type="file" accept="image/*" capture="environment" onChange={onPhoto} style={{display:"none"}}/>
    {(u.role==="e"||u.role==="a")&&<div style={{background:C.white,borderRadius:12,padding:14,border:`1px solid ${C.border}`,marginBottom:16}}><div style={{fontSize:14,fontWeight:600,marginBottom:8}}>{L.today} - {tk}</div>
    ...
    {isA&&<div style={{background:C.white,borderRadius:12,padding:14,border:`1px solid ${C.border}`,marginBottom:12}}><h3 style={{fontFamily:F.d,fontSize:15,margin:"0 0 10px",color:C.maroon}}>{tk}</h3>{allM.map(m=>{const r=att.find(a=>a.uid===m.id&&a.date===tk);return(<div key={m.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:r?C.gBg:C.bg,borderRadius:8,marginBottom:4}}>
      <div style={{width:24,height:24,borderRadius:"50%",background:m.dc||C.maroon,display:"flex",alignItems:"center",justifyContent:"center",color:C.white,fontSize:10,fontWeight:700}}>{m.n[0]}</div>
      <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600}}>{m.n}</div></div>
    ...
    {isA&&tpSlots.length>0&&<div style={{background:C.white,borderRadius:12,padding:14,border:`1px solid ${C.purple}`}}>
      <h3 style={{fontFamily:F.d,fontSize:15,margin:"0 0 12px",color:C.purple}}>🛡️ 3rd Party Guards</h3>
      {tpSlots.map(slot=>{const rec=att.find(a=>a.uid===slot.id&&a.date===tk);const isP=!!rec?.ci;return(<div key={slot.id} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 10px",background:isP?C.gBg:C.bg,borderRadius:8,marginBottom:6}}>
        <span style={{fontSize:10,padding:"2px 6px",borderRadius:4,fontWeight:700,background:slot.shift==="day"?"#FFF7ED":"#EDE9FE",color:slot.shift==="day"?C.accent:C.purple,flexShrink:0,whiteSpace:"nowrap"}}>{slot.shift==="day"?"🌅 Day":"🌙 Night"}</span>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:10,color:C.tl,marginBottom:3}}>{slot.label}</div>
          <input value={tpNames[slot.id]||""} onChange={e=>setTpNames(p=>({...p,[slot.id]:e.target.value}))} placeholder="Guard name..." style={{padding:"3px 7px",borderRadius:6,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:12,width:"100%",boxSizing:"border-box",outline:"none"}}/>
        </div>
        <button onClick={()=>tpMark(slot,true)} style={{padding:"5px 10px",borderRadius:6,border:`1px solid ${isP?C.green:C.border}`,background:isP?C.green:C.bg,color:isP?C.white:C.tl,fontFamily:F.b,fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>✓ Present</button>
        <button onClick={()=>tpMark(slot,false)} style={{padding:"5px 10px",borderRadius:6,border:`1px solid ${isP?C.border:C.red}`,background:isP?C.bg:C.rBg,color:isP?C.tl:C.red,fontFamily:F.b,fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>✗ Absent</button>
      </div>);})}
    </div>}
  </div>);
}
...
function TLV({tasks,setTasks,prop,user:u,vt,L,lang}){
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:6}}>
      <h1 style={{fontFamily:F.d,fontSize:21,fontWeight:700,color:C.maroon,margin:0}}>{vt==="mytasks"?L.myTasks:"Daily Tasks"} - {prop.sn}</h1>
      <div style={{display:"flex",gap:5}}>
        {<div style={{display:"flex",gap:2,background:C.maroonSoft,borderRadius:8,padding:2}}>{cO.map(v=><button key={v} onClick={()=>setCV(v)} style={{padding:"5px 12px",borderRadius:6,border:"none",cursor:"pointer",fontFamily:F.b,fontSize:12,fontWeight:600,background:cv===v?C.maroon:"transparent",color:cv===v?C.white:C.maroon}}>{cL[v]}</button>)}</div>}
    ...
    {vt==="mytasks"&&!isA&&<div style={{background:C.white,borderRadius:12,padding:12,border:`1px solid ${C.border}`,marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
      <div style={{width:40,height:40,borderRadius:"50%",background:C.maroon,display:"flex",alignItems:"center",justifyContent:"center",color:C.white,fontFamily:F.d,fontSize:19,fontWeight:700}}>{u.name[0]}</div>
    ...
    {vt==="mytasks"&&!isA&&<div style={{background:C.maroonSoft,borderRadius:8,padding:"7px 12px",marginBottom:10,fontSize:12}}>{L.steps}</div>}
    ...
    {fl.length===0&&<div style={{background:C.white,borderRadius:12,padding:24,textAlign:"center",border:`1px solid ${C.border}`}}><div style={{fontSize:24}}>🎉</div><div style={{fontFamily:F.d,fontSize:15,fontWeight:700,marginTop:4}}>{L.noTasks}</div></div>}
  </div>);
}
...
function AssignedTasksView({user:u,dirs,setDirs,L,setNs,setView}){
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      <h1 style={{fontFamily:F.d,fontSize:23,fontWeight:700,color:C.maroon,margin:0}}>📝 {L.directives}</h1>
    ...
    <Modal isOpen={showNew&&isSA} onClose={()=>setShowNew(false)} title={`➕ ${L.newDirective}`} size="lg">
      ...
      <textarea placeholder={L.writeTask} value={newText} onChange={e=>setNewText(e.target.value)} style={{width:"100%",padding:12,borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:14,minHeight:80,resize:"vertical",outline:"none",boxSizing:"border-box",marginBottom:8}}/>
      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:12,fontWeight:600,color:C.tl}}>📅 {L.dueDate}:</span><input type="date" value={nDue} onChange={e=>setNDue(e.target.value)} style={{padding:"6px 10px",borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:13,outline:"none"}}/></div>
        <button onClick={()=>nPhRef.current?.click()} style={{padding:"8px 14px",borderRadius:8,border:`1px dashed ${C.accent}`,background:"#FFF7ED",cursor:"pointer",fontFamily:F.b,fontSize:12,fontWeight:600,color:C.accent}}>📸 {L.addPhoto}</button>
    ...
    {isSA&&<div style={{display:"flex",gap:4,marginBottom:14,flexWrap:"wrap"}}>
      <button onClick={()=>setFilterTo("all")} style={{padding:"6px 14px",borderRadius:8,border:filterTo==="all"?`2px solid ${C.maroon}`:`1px solid ${C.border}`,background:filterTo==="all"?C.maroonSoft:C.white,cursor:"pointer",fontFamily:F.b,fontSize:12,fontWeight:600,color:C.maroon}}>All ({myDirs.length})</button>
      {admins.map(a=>{const tgt=ADMIN_TARGETS.find(t=>t.id===a.id||t.username===a.username);const col=tgt?.color||C.blue;const cnt=myDirs.filter(d=>d.to===a.id).length;return cnt>0&&<button key={a.id} onClick={()=>setFilterTo(a.id)} style={{padding:"6px 14px",borderRadius:8,border:filterTo===a.id?`2px solid ${col}`:`1px solid ${C.border}`,background:filterTo===a.id?col+"15":C.white,cursor:"pointer",fontFamily:F.b,fontSize:12,fontWeight:600,color:col}}>{a.name} ({cnt})</button>;})}
    </div>}

    {/* TASK GRID */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:12}}>
      {filteredDirs.length===0&&<div style={{gridColumn:"1/-1",background:C.white,borderRadius:12,padding:30,textAlign:"center",border:`1px solid ${C.border}`}}><div style={{fontSize:24}}>📝</div><div style={{fontFamily:F.d,fontSize:15,fontWeight:700,marginTop:4}}>{L.noDirectives}</div></div>}
...
function ATCard({dir,user:u,setDirs,L,setNs}){
  ...
  return(<div style={{background:cardBg,borderRadius:12,border:`1px solid ${bdrC}`,overflow:"hidden",borderTop:`4px solid ${topC}`}}>
    ...
    <div style={{padding:"10px 14px",borderBottom:`1px solid ${bdrC}`,display:"flex",alignItems:"center",gap:8}}>
      <div style={{width:30,height:30,borderRadius:"50%",background:mC,display:"flex",alignItems:"center",justifyContent:"center",color:C.white,fontWeight:700,fontSize:12,flexShrink:0}}>{dir.toName[0]}</div>
      <div style={{flex:1}}>
        <div style={{fontSize:13,fontWeight:700,color:mC}}>→ {dir.toName}</div>
        <div style={{display:"flex",gap:6,alignItems:"center",marginTop:1}}>
          <span style={{fontSize:10,color:C.tl}}>{dir.createdDate} · {dir.createdTime}</span>
          {dueFmt&&<span style={{fontSize:10,fontWeight:700,padding:"1px 6px",borderRadius:4,background:isOverdue?C.rBg:C.yBg,color:isOverdue?C.red:C.yellow}}>{isOverdue?"⚠️ "+L.overdue:"📅 "+(L.dueOn||"Due")} {dueFmt}</span>}
        </div>
      </div>
      <Bdg color={st.c} bg={st.b}>{st.l}</Bdg>
      {isSA&&<button onClick={async()=>{await supabase.from("assigned_tasks").delete().eq("id",dir.id);setDirs(prev=>prev.filter(d=>d.id!==dir.id));}} style={{width:22,height:22,borderRadius:6,border:"none",cursor:"pointer",background:C.rBg,color:C.red,fontSize:10,display:"flex",alignItems:"center",justifyContent:"center"}}>🗑️</button>}
    </div>

    {/* BODY */}
    <div style={{padding:"10px 14px"}}>
      <div style={{fontSize:14,lineHeight:1.6,marginBottom:6,textDecoration:isCompleted?"line-through":"none",color:isCompleted?C.green:C.text}}>{dir.text}</div>
      ...
      {status==="rejected"&&isTarget&&dir.remarksSA&&<div style={{background:C.rBg,border:`1px solid ${C.red}`,borderRadius:8,padding:"8px 12px",marginBottom:8}}>
        <div style={{fontSize:11,fontWeight:700,color:C.red,marginBottom:3}}>❌ {L.saRemarks||"SA Feedback — Rework Required"}</div>
        <div style={{fontSize:13,color:C.red}}>{dir.remarksSA}</div>
      </div>}
      ...
      {status==="approved"&&isTarget&&<div style={{background:C.gBg,border:`1px solid ${C.green}`,borderRadius:8,padding:"8px 12px",marginBottom:8}}>
        <div style={{fontSize:12,fontWeight:700,color:C.green}}>✅ {L.approvedMsg||"Approved! Complete the work on ground, then mark complete with photo proof."}</div>
      </div>}
      ...
      {status==="approval_requested"&&isTarget&&<div style={{background:"#FFF7ED",border:`1px solid ${C.accent}`,borderRadius:8,padding:"8px 12px",marginBottom:8}}>
        <div style={{fontSize:12,fontWeight:600,color:C.accent}}>🔔 {L.waitingApproval||"Approval request sent. Waiting for SA review."}</div>
      </div>}

      {/* COMPLETION PROOF */}
      {isCompleted&&<div style={{background:C.gBg,border:`1px solid #b8dcc8`,borderRadius:8,padding:"8px 12px",marginBottom:8}}>
        <div style={{fontSize:11,fontWeight:700,color:C.green,marginBottom:4}}>✅ {L.completedWork||"Completed"}</div>
        {dir.completionNote&&<div style={{fontSize:12,marginBottom:4,color:C.text}}>{dir.completionNote}</div>}
        ...
        {dir.completedAt&&<div style={{fontSize:10,color:C.tl,marginTop:4}}>{new Date(dir.completedAt).toLocaleString("en-IN")}</div>}
      </div>}
      ...
      {dir.replies.length>0&&<div style={{borderTop:`1px solid ${C.border}`,paddingTop:8,marginTop:4}}>
        {dir.replies.map((r,i)=>{
          ...
          return(<div key={r.id||i} style={{display:"flex",gap:6,marginBottom:6,padding:8,background:rBg,borderRadius:8}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:11,fontWeight:600}}>{r.by} <span style={{fontWeight:400,color:C.tl}}>{r.time}</span></div>
              {r.text&&<div style={{fontSize:12,marginTop:2}}>{r.text}</div>}
      ...
      <Modal isOpen={isSA&&showRemarks} onClose={()=>setShowRemarks(false)} title="❌ Reject — Write Remarks" size="sm">
        <textarea placeholder={L.writeRemarks||"Write remarks for admin..."} value={remarks} onChange={e=>setRemarks(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:`1px solid ${C.red}`,fontFamily:F.b,fontSize:13,minHeight:70,outline:"none",boxSizing:"border-box",marginBottom:10}}/>
      ...
      <Modal isOpen={showComplete&&isTarget&&(status==="sent"||status==="rejected"||status==="approved")} onClose={()=>setShowComplete(false)} title={`📸 ${L.markComplete||"Mark Complete"}`} size="sm">
        <div style={{fontSize:11,color:C.tl,marginBottom:8}}>Photo proof is <strong>required</strong> to complete this task.</div>
        <textarea placeholder={L.completionNote||"Completion note (optional)"} value={cNote} onChange={e=>setCNote(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:13,minHeight:60,outline:"none",boxSizing:"border-box",marginBottom:10}}/>
        <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:10}}>
          <button onClick={()=>cRef.current?.click()} style={{padding:"7px 12px",borderRadius:8,border:`2px ${cPhoto?"solid":"dashed"} ${cPhoto?C.green:C.red}`,background:cPhoto?C.gBg:"#FFF5F5",cursor:"pointer",fontFamily:F.b,fontSize:12,fontWeight:600,color:cPhoto?C.green:C.red}}>{cPhoto?"✅ Photo taken — retake?":"📸 Take Photo (Required)"}</button>
        ...
        {!cPhoto&&<div style={{fontSize:11,color:C.red,marginBottom:8}}>📸 Please take a photo to enable completion</div>}
      ...
      <Modal isOpen={showReply} onClose={()=>setShowReply(false)} title={`💬 ${L.reply||"Reply"}`} size="sm">
        <textarea placeholder={L.replyHere||"Write reply..."} value={rText} onChange={e=>setRText(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:`1px solid ${C.border}`,fontFamily:F.b,fontSize:13,minHeight:70,resize:"vertical",outline:"none",boxSizing:"border-box",marginBottom:10}}/>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <button onClick={()=>rRef.current?.click()} style={{padding:"6px 12px",borderRadius:6,border:`1px dashed ${C.accent}`,background:"#FFF7ED",cursor:"pointer",fontFamily:F.b,fontSize:11,fontWeight:600,color:C.accent}}>📸</button>
  ...
  return(<>
    ...
    {showMore&&<div style={{position:"fixed",bottom:60,left:0,right:0,background:C.white,borderTop:`1px solid ${C.border}`,zIndex:98,padding:"8px 8px 4px",boxShadow:"0 -4px 16px rgba(0,0,0,0.12)"}}>
      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
        {more.map(n=><button key={n.id} onClick={()=>{setView(n.id);setShowMore(false);}} style={{flex:"1 1 auto",minWidth:100,padding:"10px 8px",display:"flex",alignItems:"center",gap:8,border:"none",background:view===n.id?C.maroonSoft:"transparent",cursor:"pointer",borderRadius:10,color:view===n.id?C.maroon:C.text,fontFamily:F.b,fontSize:14,fontWeight:view===n.id?700:400}}>
          <span style={{fontSize:20}}>{n.i}</span><span>{n.l}</span>
          {n.badge>0&&<span style={{background:C.red,color:C.white,borderRadius:10,padding:"1px 6px",fontSize:11,fontWeight:700,marginLeft:"auto"}}>{n.badge}</span>}
        </button>)}
      </div>
      <div style={{display:"flex",gap:4,marginTop:4,padding:"4px 0",borderTop:`1px solid ${C.border}`}}>
        <button onClick={()=>setLang(lang==="en"?"hi":"en")} style={{flex:1,padding:"8px",borderRadius:8,border:`1px solid ${C.border}`,background:C.bg,fontFamily:F.b,fontSize:14,cursor:"pointer",color:C.maroon,fontWeight:600}}>🌐 {lang==="en"?L.hi:L.en}</button>
        <button onClick={()=>setShowN(true)} style={{flex:1,padding:"8px",borderRadius:8,border:`1px solid ${nC>0?C.accent:C.border}`,background:nC>0?"#FFF7ED":C.bg,cursor:"pointer",fontFamily:F.b,fontSize:14,fontWeight:600,color:nC>0?C.accent:C.tl}}>🔔{nC>0&&<span style={{background:C.red,color:C.white,borderRadius:10,padding:"0 5px",fontSize:11,marginLeft:4}}>{nC}</span>}</button>
        <button onClick={onLogout} style={{flex:1,padding:"8px",borderRadius:8,border:`1px solid ${C.red}`,background:C.rBg,color:C.red,fontFamily:F.b,fontSize:14,fontWeight:600,cursor:"pointer"}}>🚪</button>
      </div>
    </div>}
    <div className="mobile-bottom-nav" style={{position:"fixed",bottom:0,left:0,right:0,height:60,background:C.white,borderTop:`1px solid ${C.border}`,display:"flex",alignItems:"stretch",zIndex:99,paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      {main.map(n=><button key={n.id} onClick={()=>setView(n.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,border:"none",background:"transparent",cursor:"pointer",position:"relative",padding:"4px 0"}}>
        ...
        {n.badge>0&&<span style={{position:"absolute",top:4,right:"15%",background:C.red,color:C.white,borderRadius:10,padding:"0 5px",fontSize:11,fontWeight:700}}>{n.badge}</span>}
      </button>)}
...
export default function App(){
  ...
  return(<div style={{fontFamily:F.b,background:C.bg,minHeight:"100vh",color:C.text}}>
    ...
    <div className="main-content" style={{marginLeft:isMobile?0:185,padding:isMobile?"0 12px 18px":"0 18px 18px",minHeight:"100vh"}}>
      {pm&&previewDbUser&&<div style={{background:`linear-gradient(90deg,${C.blue},${C.maroon})`,color:C.white,padding:"8px 14px",borderRadius:10,marginTop:10,marginBottom:4,display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:6}}><span>👁️</span><span style={{fontSize:13,fontWeight:700}}>{L.previewAs}: {eU.name} ({eU.role==="a"||!!findAT(eU)?L.admin:L.staff} — {PROPS[eU.prop]?.sn||eU.prop||"All"})</span></div><button onClick={()=>{setPM(false);setPAs("");sV("dashboard");}} style={{padding:"4px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.5)",background:"rgba(255,255,255,0.15)",color:C.white,fontFamily:F.b,fontSize:11,fontWeight:700,cursor:"pointer"}}>{L.previewOff}</button></div>}
